...
class NewJisi:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&Add new JiSi')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('NewJisi', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/new_jisi/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Add new Jisi'),
            callback=self.run,
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg = NewJisiDialog()
            self.dlg.cmbSpecies.currentTextChanged.connect(self.Jisi_cmbSpecies_change)
            # QMessageBox.information(self.dlg, "message", "Should only run at the beginning.")
        ...
        if result:
            ...
            Lat_value = self.dlg.spbLatitude.value()
            Buffer_value = self.dlg.spbBuffer.value()
            Species_value = self.dlg.cmbSpecies.currentText()
            Status_value = self.dlg.cmbStatus.currentText()
            Last_value = self.dlg.dteLast.date()
            QMessageBox.information(self.dlg, "message", "New Nest ID:{}\n\nLngitude:{}\nLatitude:{}\nSpecies:{}\nBuffer:{}\nStatus:{}\nLast survey:{}.".format(NestID_value, Lng_value, Lat_value, Species_value, Buffer_value, Status_value, Last_value))
            ftrNest = QgsFeature(lyrNests.fields())
            ftrNest.setAttribute("long_x_dd", Lng_value)
            ftrNest.setAttribute("lat_y_dd", Lat_value)
            ftrNest.setAttribute("recentspec", Species_value)
            ftrNest.setAttribute("buf_dist", Buffer_value)
            ftrNest.setAttribute("recentstat", Status_value)
            ftrNest.setAttribute("lastsurvey", Last_value)
            ftrNest.setAttribute("Nest_ID", NestID_value)
            geom = QgsGeometry(QgsPoint(Lng_value,  Lat_value))
            ftrNest.setGeometry(geom)

            # insert ftrNest into Nest layer
            prv = lyrNests.dataProvider()
            prv.addFeatures([ftrNest])
            lyrNests.reload()

            # insert ftrNest into Buffer layer
            prv = lyrBuffer.dataProvider()
            buffer = geom.buffer(Buffer_value, 10)
    ...
    def Jisi_cmbSpecies_change(self, species):
        if species == "tuantaun Pig":
